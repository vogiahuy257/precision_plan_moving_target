...
constexpr double MARKER_TIMEOUT_SEC = 5.0;  // 300 ms
    ...
    : Node("aruco_tracker_node")
{
    ...
    get_parameter_or("image_topic", image_topic, std::string("/world/aruco/model/x500_mono_cam_down_0/link/camera_link/sensor/imager/image"));
    get_parameter_or("camera_info_topic", camera_info_topic, std::string("/world/aruco/model/x500_mono_cam_down_0/link/camera_link/sensor/imager/camera_info"));
    ...
    _kalman_reset_pub = create_publisher<std_msgs::msg::String>("/reset", qos);
    ...
    try {
        ...
        if (!ids.empty()) {
            ...
            for (size_t i = 0; i < ids.size(); ++i) {
                if (ids[i] != _param_aruco_id) continue;
                found = true;

                cv::Vec3d rvec, tvec;
                
                // Optimized: Direct PnP with distortion handling
                ...
                pose_msg.header = msg->header; // Critical: Copy timestamp & frame_id
                // ===== GIMBAL CORRECTION (CAMERA FRAME ONLY) =====
                ...
                _last_seen_time = msg->header.stamp;
                break;
            }
        // ===== TIMEOUT RESET =====
        if (_has_valid_pose && !found) {
            rclcpp::Time now = msg->header.stamp;
            double dt = (now - _last_seen_time).seconds();
